...
export function buildPlugins(root) {
  ...
  const index = { release, entries: cat.entries.map(({ id, kind, relative, category }) => ({ id, kind, path: relative, category })), aliases: cat.aliases, commands: Object.fromEntries([...commands].map(([k,v]) => [v.id,k])) };
