...
try {
 ...
 if(args.includes('--help')) {console.log('Usage: node scripts/live-eval.mjs --output DIR --total-cap-usd N [--prior-cost-usd N] [--case-cap-usd 0.5] [--model MODEL]\nSends the eight fictional scenarios and Foreman content to Claude. Uses a fresh workspace for each scenario. Existing output directories are refused.');process.exit(0);}
 for(let i=0;i<args.length;i+=2) {
  const key=args[i].replace(/^--/,'');
  if(!['output','total-cap-usd','prior-cost-usd','case-cap-usd','model'].includes(key)||args[i+1]===undefined)throw new Error('Invalid or incomplete option');
  opts[key]=args[i+1];
 }
 ...
 const cases=JSON.parse(fs.readFileSync(path.join(root,'tests/advisory-scenarios.json'),'utf8')).filter(c=>c.prompt);
 ...
 for(const scenario of cases) {
  ...
  if(cost===undefined||result.error||result.status!==0)throw new Error('Uncertain cost, timeout, or process failure; no further calls started');
 }
