...
trigger_patterns:
  ...
  - Launch timeline is slipping
  - Don't know where to start with launch
routes_to:
  diagnostics:
...
IF intent == beta-transition:
  → diagnostics/market-entry-diagnosis
  → playbooks/first-100-customers-playbook
  → playbooks/first-100-customers-playbook.md
  → skills/frameworks/net-promoter-score.md

IF intent == product-line-launch:
  → diagnostics/market-entry-diagnosis
  → playbooks/launch-playbook
  → skills/frameworks/growth-share-matrix.md
