---
name: "clarity-and-concision"
description: "Teaches techniques for achieving relentless clarity and ruthless concision in business writing, including the 20-word sentence rule, concrete detail over abstraction, word economy tactics, and the reader's perspective test. Features extensive before/after examples demonstrating how bloated corporate prose can be cut by 50-70% without losing meaning. Use when tightening any business document, eliminating wordiness, improving readability scores, preparing executive summaries, or when writing feels vague or overlong."
version: "1.1.0"
category: "writing"
complexity: "basic"
stage:
  - idea
  - validation
  - early-traction
  - growth
  - scale
tags:
  - writing
  - clarity
  - concision
  - word-economy
  - readability
  - editing
  - plain-language
  - business-writing
related_skills:
  - revising-and-editing
  - plain-language-and-flow
  - writing-with-purpose
  - business-writing-for-leaders
author: "Fatih Guner"
---

Read [runtime](../../content/RUNTIME.md) and [advisory rules](../../content/policies/advisory.md) before applying this skill. Other Foreman layers and the catalog are in `../../content/`, relative to this SKILL.md.


# Clarity and Concision

Consider a mid-sized company where 200 professionals each write an average of 10 internal messages per day, each message averaging 150 words. That is 300,000 words produced daily -- roughly three novels. Now suppose that 30% of those words are unnecessary. The organisation is generating 90,000 wasted words every working day. At an average reading speed of 250 words per minute, that is 360 person-hours per day spent reading words that should never have been written. Six full-time employees, year-round, doing nothing but reading padding. The financial cost of corporate verbosity is not metaphorical. It is measurable, and it is staggering.

Clarity and concision are not aesthetic preferences. They are economic imperatives.

---

## The Framework

### Clarity as Courage

Clarity is a double-edged sword. When a writer takes a position or recommends a course of action, the writing is clear -- and the writer is exposed. People who do not want to commit make their writing muddy on purpose. They leave room to claim credit for good outcomes and deny responsibility for bad ones. Readers, however, perceive this not as strategic ambiguity but as spinelessness. Clear writing requires taking a stand.

The test for clarity is simple: show the draft to a colleague who has no context and ask them to identify the three main points. If they cannot do so accurately after one read-through, the writing is not clear enough.

### The Research Case for Less

The intuition that shorter is better has been validated experimentally, at scale. *Writing for Busy Readers* reports a field experiment in which two versions of an email were sent to 7,002 school board members across the United States requesting completion of a short online survey. One email contained 127 words; the other contained 49 words. The 49-word version produced nearly double the response rate -- 4.8% versus 2.7%. The survey itself was identical in both conditions. Some recipients likely looked at the longer email and declined to engage at all. Others read partway through and never reached the request. Still others used the email's length as a proxy for how long the survey would take: 29% of those who saw the concise version estimated the survey at under five minutes, compared with just 15% of those who saw the longer version.

The average American adult reads nonfiction at approximately 240 words per minute -- four words per second. At that rate, every unnecessary sentence costs measurable time, compounded across every recipient. The arithmetic is unforgiving: a 500-word email sent to 200 people that could have been written in 250 words wastes over two hours of collective reading time. Scale that to a year of weekly all-hands updates and the cost becomes a line item no one budgets for but everyone pays.

Perhaps the most revealing finding is how poorly writers self-correct. Gabrielle Adams and colleagues at the University of Virginia asked participants to summarise a short article and then edit their summaries to improve them. Eighty-three percent of participants *added* words during the editing pass rather than removing them. The same pattern held across topics from travel itineraries to patents. The instinct to accrete rather than subtract appears to be a cognitive default -- one that writers must consciously override.

Blaise Pascal captured the asymmetry centuries ago: "I would have written a shorter letter if I'd had more time." The line has been attributed to Twain, Locke, and a dozen others, which only confirms how universally writers recognise the problem -- and how rarely they solve it.

### The 20-Word Rule

Research has confirmed repeatedly that the optimal average for readable sentences is no more than 20 words. This does not mean every sentence should be exactly 20 words -- variety in sentence length creates rhythm and holds interest. Some sentences should be very short. Others can be longer and more complex, building layered arguments. But the average, across a document, should hover around 20.

**NOT THIS:**
> Efficiency measures that have been implemented by the company with strong involvement of senior management have generated cost savings while at the very same time assisting in the building of a culture that is centered around the value of efficiency. [40 words]

**BUT THIS:**
> Our senior management team has cut costs and made the company more efficient. We expect to be more profitable for the next four quarters. [24 words]

The first version says less with more. The second says more with less.

### Show, Don't Tell

Concrete business writing is persuasive because it is evidence-based, clear, and memorable. Abstract assertions invite scepticism; specific details earn credibility.

| NOT THIS | BUT THIS |
|----------|----------|
| He was a bad boss. | He got a promotion based on his assistant's detailed reports, but then -- despite the company's record profits -- denied that assistant even routine cost-of-living raises. |
| The company lost its focus and floundered. | The CEO acquired five unrelated subsidiaries -- as far afield as a paper company and a retailer of children's toys -- and then couldn't service the $26 million in debt. |

The vague versions are opinions. The specific versions are evidence that leads readers to draw the same conclusions -- independently, and therefore more convincingly.

### Word Economy: Three Surgical Techniques

Wordiness operates on multiple levels, from bloated sentences to verbose phrases to unnecessary padding. Three techniques eliminate the majority of excess:

**Technique 1: Delete prepositions (especially "of").**
- *April of 2013* becomes *April 2013*
- *point of view* becomes *viewpoint*
- *the purpose of this evaluation* becomes *this evaluation's purpose*

**Technique 2: Replace -ion nouns with verbs.**
- *was in violation of* becomes *violated*
- *provided protection to* becomes *protected*
- *made a determination* becomes *determined*

**Technique 3: Replace be-verbs with stronger verbs.**
- *was hanging* becomes *hung*
- *is indicative of* becomes *indicates*
- *are in agreement* becomes *agree*

These three rules, applied mechanically, typically reduce word count by 20-40%.

### Before/After: The Compound Effect

| NOT THIS | Words | BUT THIS | Words |
|----------|-------|----------|-------|
| The trend in the industry is toward self-generation by some companies of their own websites, and Internet technology is changing the nature of training necessary to acquire the skill of website development at an acceptable level of sophistication, so that this activity can more and more be handled in-house. | 49 | Since Internet technology makes it easier than ever to develop sophisticated websites, some companies now develop their own in-house. | 19 |
| We are unable to fill your order at this point in time because there is an ongoing dock strike that affects our operations. | 23 | We cannot fill your order right now because of the dock strike. | 12 |
| I am writing in response to a number of issues that have arisen with regard to the recent announcement that there will be an increase in the charge for the use of our lobby computers. | 35 | You may have heard that we're raising the fees for using our lobby computers. | 14 |
| The greater number of these problems can readily be dealt with in such a way as to bring about satisfactory solutions. | 21 | Most of these problems can be readily solved. | 8 |

In the most extreme case, 49 words become 19 -- a 61% reduction with no loss of meaning. The shorter versions are not merely more efficient; they are more persuasive. Readers trust writers who respect their time.

### Eliminate Padding

Certain phrases contribute nothing but the illusion of seriousness:

- *in this connection it might be observed that* -- delete entirely
- *it is important to bear in mind that* -- delete entirely
- *it is interesting that* -- delete entirely
- *it is notable that* -- delete entirely
- *it is worthwhile to note that* -- delete entirely
- *it should be pointed out that* -- delete entirely

These phrases are throat-clearing. They signal to the reader that the writer is about to say something -- rather than simply saying it.

---

## Prompts

**Prompt 1 -- Word Economy Audit:**
> Here is a business document: [paste text]. Apply the three word-economy techniques: (1) delete unnecessary prepositions, (2) replace -ion nouns with verbs, (3) replace be-verbs with stronger alternatives. Also identify and eliminate all padding phrases. Show the original word count, the revised text, and the new word count. Calculate the percentage reduction.

**Prompt 2 -- Clarity Stress Test:**
> Pretend you are an intelligent reader with no prior context about my company or project. Read this document: [paste text]. After one read-through: (1) What are the three main points? (2) What action is the writer requesting? (3) Where did you get confused? (4) Which passages could be misinterpreted? (5) Where did you have to re-read a sentence? Identify every clarity failure and suggest specific revisions.

**Prompt 3 -- Show Don't Tell Conversion:**
> Here is a draft containing abstract assertions: [paste text]. Identify every sentence that "tells" rather than "shows" -- every opinion, generalisation, or claim that lacks supporting evidence. For each, either suggest a concrete, specific replacement or flag it as needing data I should gather. The goal is a document where every claim is supported by evidence.

**Prompt 4 -- Sentence Length Analysis:**
> Analyse the sentence lengths in this document: [paste text]. Calculate the average. Identify the five longest sentences and suggest how to break them up. Identify any passages where all sentences are similar in length and suggest how to vary the rhythm. Target an average of 20 words or fewer.

**Prompt 5 -- The Subtraction Edit:**
> Most editors instinctively add words when revising. Reverse that default. Take this draft: [paste text]. Your only permitted operation is deletion -- remove words, sentences, and entire paragraphs. Do not add a single word. Do not rephrase; only cut. Produce the shortest version that preserves every essential idea and action request. Show the original word count, the revised word count, and the percentage reduction. Then list, in a table, every deleted passage with a one-line justification for why it was expendable.

---

## Use Cases

**Idea-Stage Founder Rewriting a Landing Page**

A founder's landing page opens with: "We aspire to be a partner primarily concerned with providing our clients the maximal acquisition of future profits and assets and focus mainly on clients with complex and multi-product needs, large and midsized corporate entities, individual or multiple entrepreneurial agents, and profit-maximising institutional clients." After applying the clarity and concision framework: "We're a client-focused firm dedicated to making sure you get the most out of our services. Our client base includes individual entrepreneurs, midsized companies, and large corporations." The original was 46 words of impenetrable corporate fog. The revision is 30 words that a customer can actually understand -- and that a search engine can parse.

**Growth-Stage Company Tightening Investor Updates**

A SaaS company's monthly investor update runs 2,500 words. Most investors skim it in under two minutes. Applying word economy techniques across three consecutive updates: Update 1 drops from 2,500 to 1,800 words (28% reduction). Update 2 drops to 1,400 words as the writer internalises the preposition and be-verb rules. Update 3 reaches 1,100 words -- the same information in 56% less space. Investor engagement (measured by reply rate) increases by 40%, because the update now respects the audience's time.

**Early-Traction Nonprofit Doubling Newsletter Engagement**

A nonprofit sends a weekly newsletter to 50,000 subscribers linking to six curated resources. The editorial team, composed of professional writers, drafts a 566-word newsletter with context, supporting quotes, and background detail for each link. Applying the subtraction edit -- cutting supporting detail that is relevant but not essential, removing author quotes, and eliminating scene-setting paragraphs -- the team produces a 275-word version preserving all six resource links. The concise version generates twice as many link clicks as the original. The lost detail represented the kind of supplementary material that writers value and readers skip. As one researcher in the *Writing for Busy Readers* study observed: readers who received the shorter version did not report feeling less informed -- they simply engaged more.

---

## Anti-Patterns

1. **Confusing Brevity with Concision.** Brevity means being short. Concision means being short without losing substance. A one-sentence summary that omits critical context is brief but not concise. The goal is to include everything necessary and nothing else.

2. **Using Complexity as a Status Signal.** Some writers equate long sentences and Latinate vocabulary with intelligence. Readers equate it with obscurity. A 136-word sentence scoring 8.2 on the Flesch Reading Ease scale does not signal sophistication -- it signals that the writer is either hiding something or has not thought clearly about the subject.

3. **Cutting Too Aggressively.** Concision taken to the extreme produces telegraphic prose that sounds curt and loses the writer's voice. The goal is to remain "faithful to the sounds and rhythms of normal, down-to-earth English" while eliminating waste.

4. **Editing for Concision Before Revising for Structure.** Cutting 30% of the words from a structurally flawed document produces a shorter structurally flawed document. Always revise the macro structure first, then edit for concision.

5. **Telling Instead of Showing.** Asserting that "the company performed poorly" is brief, but it is not persuasive. The concrete version -- "The CEO acquired five unrelated subsidiaries and couldn't service the $26 million in debt" -- is longer but incomparably more effective. Concision serves clarity; it does not replace evidence.

---

## By Stage

| Stage | Focus | Key Difference |
|-------|-------|----------------|
| Idea | Pitch compression | Every word on a pitch deck slide or one-pager must justify its existence. At the idea stage, the entrepreneur has less content and more temptation to pad. Concision here means saying less but meaning more. |
| Validation | Data-driven concision | Replace assertions with numbers. "Strong traction" becomes "47 paying customers in 3 months with 12% week-over-week growth." Specific data is inherently concise because it replaces paragraphs of qualitative hedging. |
| Early Traction | Communication volume management | The sheer volume of writing at this stage makes concision a time-management tool. Cutting 30% of words across all documents saves hours per week for both writer and readers. |
| Growth | Readability standards | Establish organisational standards: maximum average sentence length, maximum word count for recurring document types, required use of headings for documents over 500 words. Concision becomes policy. |
| Scale | Brand voice consistency | At scale, every external communication reflects the brand. A house style that mandates concision prevents the verbal inflation that accompanies corporate growth and bureaucratic layering. |

---

## Output Template

```markdown
# Clarity and Concision Audit: [Document Title]

## Readability Metrics
- **Original word count:** [X]
- **Target word count:** [Y] (reduction target: [Z]%)
- **Average sentence length:** [X words] (target: 20 or fewer)
- **Longest sentence:** [X words] (location: paragraph [N])
- **Flesch Reading Ease score:** [X] (target: 50+)

## Clarity Assessment
- **Main points identifiable on first read?** [Yes/No]
- **Ambiguous passages:** [List with paragraph numbers]
- **Abstract assertions needing evidence:** [List]
- **Jargon needing plain-language replacement:** [List]

## Concision Edits
### Prepositions Removed
| Original | Revised |
|----------|---------|
| [phrase] | [tighter phrase] |

### -ion Nouns Replaced with Verbs
| Original | Revised |
|----------|---------|
| [phrase] | [verb form] |

### Be-Verbs Replaced
| Original | Revised |
|----------|---------|
| [phrase] | [stronger verb] |

### Padding Eliminated
| Phrase Removed | Location |
|----------------|----------|
| [padding phrase] | [paragraph N] |

## Final Metrics
- **Revised word count:** [X]
- **Actual reduction:** [Y]%
- **Revised average sentence length:** [X words]
```

---

## Related Skills

- **[Revising and Editing](../../content/skills/writing/revising-and-editing.md)** -- Concision editing is one component of the broader editing discipline; this companion skill provides the full revision-then-editing sequence.
- **[Plain Language and Flow](../../content/skills/writing/plain-language-and-flow.md)** -- Plain language and concision are complementary disciplines: one eliminates jargon, the other eliminates waste.
- **[Writing with Purpose](../../content/skills/writing/writing-with-purpose.md)** -- A clear purpose prevents unnecessary content from entering the draft in the first place, making subsequent concision editing less onerous.
- **[Reader Psychology](../../content/skills/writing/reader-psychology.md)** -- The cognitive science of working memory limits and triage behaviour explains why every unnecessary word has a measurable cost in reader comprehension.
- **[Business Writing for Leaders](../../content/skills/leadership/business-writing-for-leaders.md)** -- Leaders who model concise writing set the standard for organisational communication culture.
- **[Writing Effective Emails](../../content/skills/writing/writing-effective-emails.md)** -- Email is the medium where concision has the highest impact because readers triage ruthlessly and mobile screens punish verbosity.
